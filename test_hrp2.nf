...
process translate {
    ...
    input:
        tuple val(sample), path(assembly) from assemblyout
    
    output:
        tuple val(sample), path("${sample}_translated.fasta") into translatedseq
...
process countpattern {
    ...
    input:
        tuple val(sample), path(transfasta) from translatedseq
